...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="num of residual layers",
    )
    parser.add_argument(
        "--non_local",
        dest="non_local",
        action="store_true",
        help="if use non-local layers",
    )
...
def main(args):
    ...
    if args.evaluate:
        ...
        print(
            "Protocol #1  (minMPJPE) action-wise average: {:.2f} (mm)".format(
...
def train(data_loader, model_mean, gauss_model, optimizer, device):
    ...
    for i, (targets_3d, inputs_2d, _) in enumerate(data_loader):
        ...
        loss = -gauss_model.log_prob(
            outputs_3d_mean.view(num_poses, -1), targets_3d.view(num_poses, -1)
        ).mean()
        ...
        if i % 1000 == 0:
            ...
            save_ckpt(
                {"state_dict": gauss_model.state_dict()},
                "./",
                suffix="iso_gaussian_log_prob",
            )
...
def evaluate(data_loader, model_mean, gauss_model, device):
    ...
    for i, (targets_3d, inputs_2d, _) in enumerate(data_loader):
        ...
        targets_3d, inputs_2d = targets_3d.to(device), inputs_2d.to(
            device
        )  # Remove hip joint for 3D poses
        ...
        samples = gauss_model.sample(
            outputs_3d_mean.view(num_poses, -1), n_samples=(num_poses, 200)
        )
        ...
        mpjpe_error = (
            mpjpe(samples, targets_3d.unsqueeze(0).repeat(200, 1, 1, 1)).min(0).values
        )
